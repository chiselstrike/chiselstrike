...
pub fn infer_filter(
    call_expr: &CallExpr,
    symbols: &Symbols,
) -> (Option<Box<QOperator>>, Option<Index>) {
    ...
    let arrow = match &*arg.expr {
        ...
        Expr::Object(object_lit) => {
             ...
             */
            let index = Index::from_object_lit(entity_type, object_lit);
    ...
    let index = Index::from_filter(entity_type, &filter);
    (Some(Box::new(QOperator::Filter(filter))), Some(index))
}
