...
pub async fn params_get_typed(c: TestContext) {
    ...
    assert_eq!(c.chisel.get_json("/dev/string/abc").await, json!(["string", "abc"]));
    assert_eq!(c.chisel.get_json("/dev/string/null").await, json!(["string", "null"]));
    assert_eq!(c.chisel.get_json("/dev/string/123").await, json!(["string", "123"]));

    assert_eq!(c.chisel.get_json("/dev/number/10").await, json!(["number", 10]));
    assert_eq!(c.chisel.get_json("/dev/number/0.125").await, json!(["number", 0.125]));
    assert_eq!(c.chisel.get_json("/dev/number/-10.5").await, json!(["number", -10.5]));
    assert_eq!(c.chisel.get_json("/dev/number/3e2").await, json!(["number", 300]));
    assert_eq!(c.chisel.get_json("/dev/number/0.5foo").await, json!(["number", 0.5]));
    assert_eq!(c.chisel.get_json("/dev/number/foo").await, json!(["number", null]));
    assert_eq!(c.chisel.get_json("/dev/number/infinity").await, json!(["number", null]));

    assert_eq!(c.chisel.get_json("/dev/int/10").await, json!(["int", 10]));
    assert_eq!(c.chisel.get_json("/dev/int/1234567").await, json!(["int", 1234567]));
    ...
    assert_eq!(c.chisel.get_json("/dev/int/foo").await, json!(["int", null]));
    assert_eq!(c.chisel.get_json("/dev/int/42foo").await, json!(["int", 42]));
    assert_eq!(c.chisel.get_json("/dev/int/42.5").await, json!(["int", 42]));
    assert_eq!(c.chisel.get_json("/dev/int/3e2").await, json!(["int", 3]));

    assert_eq!(c.chisel.get_json("/dev/bool/true").await, json!(["bool", true]));
    assert_eq!(c.chisel.get_json("/dev/bool/1").await, json!(["bool", true]));
    assert_eq!(c.chisel.get_json("/dev/bool/false").await, json!(["bool", false]));
    assert_eq!(c.chisel.get_json("/dev/bool/0").await, json!(["bool", false]));
    assert_eq!(c.chisel.get_json("/dev/bool/something").await, json!(["bool", true]));
}
...
pub async fn params_get_wrong(c: TestContext) {
    ...
    assert_eq!(c.chisel.get_text("/dev/missing/string").await, "Error: Undefined parameter 'x'");
    assert_eq!(c.chisel.get_text("/dev/missing/number").await, "Error: Undefined parameter 'x'");
    assert_eq!(c.chisel.get_text("/dev/missing/int").await, "Error: Undefined parameter 'x'");
    assert_eq!(c.chisel.get_text("/dev/missing/bool").await, "Error: Undefined parameter 'x'");
}
...
pub async fn method_shorthands(c: TestContext) {
    ...
    assert_eq!(c.chisel.request_json(Method::GET, "/dev/route", "").await, json!(["GET"]));
    assert_eq!(c.chisel.request_json(Method::POST, "/dev/route", "").await, json!(["POST"]));
    assert_eq!(c.chisel.request_json(Method::PUT, "/dev/route", "").await, json!(["PUT"]));
    assert_eq!(c.chisel.request_json(Method::DELETE, "/dev/route", "").await, json!(["DELETE"]));
    assert_eq!(c.chisel.request_json(Method::PATCH, "/dev/route", "").await, json!(["PATCH"]));
}
...
pub async fn method_manual(c: TestContext) {
    ...
    assert_eq!(c.chisel.request_json(Method::GET, "/dev/route1", "").await, json!([1, "get"]));
    assert_eq!(c.chisel.request_json(Method::POST, "/dev/route1", "").await, json!([1, "post"]));
    assert_eq!(c.chisel.request_status(Method::DELETE, "/dev/route1", "").await, 405);

    assert_eq!(c.chisel.request_json(Method::GET, "/dev/route2", "").await, json!([2]));
    assert_eq!(c.chisel.request_json(Method::POST, "/dev/route2", "").await, json!([2]));
    assert_eq!(c.chisel.request_status(Method::DELETE, "/dev/route2", "").await, 405);

    assert_eq!(c.chisel.request_json(Method::DELETE, "/dev/route3", "").await, json!([3, "delete"]));
    assert_eq!(c.chisel.request_json(Method::PATCH, "/dev/route3", "").await, json!([3, "patch"]));
    assert_eq!(c.chisel.request_status(Method::GET, "/dev/route3", "").await, 405);

    assert_eq!(c.chisel.request_json(Method::GET, "/dev/route4", "").await, json!([4]));
    assert_eq!(c.chisel.request_json(Method::POST, "/dev/route4", "").await, json!([4]));
    assert_eq!(c.chisel.request_json(Method::PATCH, "/dev/route4", "").await, json!([4]));
}
...
pub async fn errors(c: TestContext) {
    ...
    assert_eq!(c.chisel.get_body("/dev/route1").await, (500, Bytes::from("")));
    assert_eq!(c.chisel.get_body("/dev/route2").await, (500, Bytes::from("")));
    assert_eq!(c.chisel.get_body("/dev/route3").await, (405, Bytes::from("")));
    assert_eq!(c.chisel.get_body("/dev/nonexistent").await, (404, Bytes::from("")));
}
