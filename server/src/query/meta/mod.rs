...
impl MetaService {
    ...
    pub async fn connect(meta_uri: &str) -> Result<Self, MetaServiceError> {
        let opts =
            AnyConnectOptions::from_str(meta_uri).map_err(MetaServiceError::ConnectionFailed)?;
        let pool = AnyPoolOptions::new()
            .connect(meta_uri)
            .await
            .map_err(MetaServiceError::ConnectionFailed)?;
        Ok(MetaService { opts, pool })
    }

    /// Create the schema of the underlying metadata store.
    pub async fn create_schema(&self) -> Result<(), MetaServiceError> {
        ...
        for table in tables {
            ...
            conn.execute(query)
                .await
                .map_err(MetaServiceError::ExecuteFailed)?;
        }
        Ok(())
    }
    ...
    pub async fn load_type_system<'r>(&self) -> Result<TypeSystem, MetaServiceError> {
        ...
        let rows = query
            .fetch_all(&self.pool)
            .await
            .map_err(MetaServiceError::FetchFailed)?;
    ...
    async fn load_type_fields(
        ...
        ts: &TypeSystem,
        type_id: i32,
    ) -> Result<Vec<Field>, MetaServiceError> {
    ...
    pub async fn insert(&self, ty: ObjectType) -> Result<(), MetaServiceError> {
    ...
    pub async fn remove(&self, type_name: &str) -> Result<(), MetaServiceError> {
