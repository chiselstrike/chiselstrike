...
pub async fn migrate_schema_step(
    ...
    // There is a bit of a mess in the schema versions. The legacy versions are:
    //
    // - "empty" is the special version when the database is completely empty ("the beginning of
    // time")
    // - "0" is the schema at the point that we started versioning the database (i.e. before release 0.7)
    // - "0.7" is the schema at any point between releases 0.7 and 0.12 (inclusive)
    // - "1" is the schema at release 0.12
    //
    // Further versions are numbered as integers "2", "3", ... and they are not tied to releases.
    Ok(match old_version {
        ...
        "0.7" => {
            migrate_from_0_7_to_1(ctx).await?;
            Some("1")
        }
        "1" => {
...
async fn migrate_from_0_7_to_1(ctx: &mut MigrateContext<'_, '_>) -> Result<()> {
    // there were many modifications to the database between Chiselstrike releases 0.7 and 0.12;
...
async fn migrate_to_2(ctx: &mut MigrateContext<'_, '_>) -> Result<()> {
    execute_stmt(
        ctx,
        sea_query::Table::create()
            .table(Modules::Table)
    ...
    let source_rows = source_rows
        .into_iter()
        .map(|row| migrate_to_2::SourceRow {
            path: row.get(0),
            code: row.get(1),
        })
        .collect();

    let module_rows = migrate_to_2::migrate_sources(source_rows);
