...
#[deno_core::op]
fn op_chisel_get_version_type_system(state: &mut deno_core::OpState) -> SimpleTypeSystem {
    let ts = state.borrow::<WorkerState>().version.type_system.clone();

    let custom_entities = ts
        .custom_types
        .iter()
        .map(|(name, ty)| (name.to_owned(), simplify_object_type(ty.object_type())))
        .collect();
    let mut builtin_entities = HashMap::default();
    for (name, ty) in &ts.builtin.types {
        if let Type::Entity(entity) = ty {
            builtin_entities.insert(name.to_owned(), simplify_object_type(entity.object_type()));
        }
    }
    SimpleTypeSystem {
        custom_entities,
...
struct SimpleTypeSystem {
    custom_entities: HashMap<String, SimpleEntity>,
    builtin_entities: HashMap<String, SimpleEntity>,
}
